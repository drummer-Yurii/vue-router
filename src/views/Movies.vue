...
<template>
    ...
    <div class="m-2 p-2">
        ...
        <div class="max-w-sm mx-auto" v-if="moviesStore.isLoading"><span class="text-2xl font-bold text-indigo-700">Is Loading...</span></div>
        <div class="grid grid-cols-3 gap-4" v-else>
            <MovieCard v-for="movie in moviesStore.filterMovies" :key="movie.id" :movie="movie" />
        </div>
    </div>
</template>